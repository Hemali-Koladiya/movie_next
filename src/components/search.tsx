        ...
        {isInputFocused && (
          <ul className="absolute w-full mt-2 bg-white border border-gray-200 rounded-md shadow-lg max-h-60 overflow-auto z-10 text-start">
            {[...staticTrendingSuggestions, ...dynamicSuggestions].map((suggestion, index) => (
              <li
                key={index}
                className={`flex items-center px-4 py-2 hover:bg-gray-100 cursor-pointer ${selectedIndex === index ? 'bg-gray-100' : ''}`}
                onClick={() => {
                  setQuery(suggestion.title);
                  onSearch(suggestion.title);
                  setDynamicSuggestions([]);
              ...
              >
                {index < staticTrendingSuggestions.length ? (
                  <>
                    <svg width="800px" height="800px" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg" className='w-6 h-6 mr-2 text-red-100'>
